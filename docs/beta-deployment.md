...
- Deployed on: 2026-08-20
- Deployed commit: `1cce0cc0699de31ced33873ad8d03db9a74bfc82`
...
- 2026-08-20 repository-authority upgrade: implementation `origin/master` was fast-forwarded through `ce11a85` and then deployment fix `1cce0cc0699de31ced33873ad8d03db9a74bfc82`. The first deployment attempt at `ce11a85` failed closed before app switching because `VISUAL-QC-UPGRADE-PREFLIGHT-V1` rejected live source version `84b391713bc1ce34e8a75482435d6b2ec9d27430`; rollback restored `motherboard-repair-visual-qc` online and left production at `84b3917`. Commit `1cce0cc` records `84b3917` as a reviewed no-schema-change source and its local preflight tests passed. The successful deployment used a 71,992,666-byte allowlisted runtime archive bound to SHA-256 `54f29832d8fca219e75ad4d6d95e5c0e1a1286c8aa1d8836db7b8603c3ff8314`, 37 runtime paths, and runtime-manifest SHA-256 `29042232482dcac0cca571d3124eb7b83420b73083be0097f538ae176db6647a`.
- Post-switch P4 for `1cce0cc`: `VERSION` matches `1cce0cc0699de31ced33873ad8d03db9a74bfc82`; both motherboard PM2 services are online; local health, public health and public technician route return `200`; beta root redirects to the public technician entry; unauthenticated internal Visual-QC workbench remains protected; upgrade preflight reports source `84b3917`, target `1cce0cc`, and all ten checks passed. Chrome/Playwright desktop `1440x900` and mobile `390x844` loaded the public technician entry with model copy present, no page errors, no horizontal overflow, and screenshots retained under the Codex task output directory. The only desktop console resource error was an uncaptured 404 consistent with the historical non-blocking favicon class.
- 2026-09-08 production storage cleanup: after a read-only joint CSAT/mainboard audit and explicit Milo authorization, removed only the audited deployment-cache candidates from `/opt/motherboard-repair-beta`: stale flat deploy artifacts, 17 old Visual-QC venv directories, and 21 old rollback directories. The cleanup kept the active app, persistent data, active `venv-visual-qc` symlink and target, previous production venv, and the latest two successful rollback packages. Project root usage fell from `6,877,039,212` bytes to `785,262,532` bytes, freeing the audited `6,091,776,680` bytes. Root filesystem availability rose from `8,900,206,592` bytes to `15,150,202,880` bytes during the cleanup run. Post-cleanup P4 kept production at `1cce0cc0699de31ced33873ad8d03db9a74bfc82`; public technician route returned `200`, unauthenticated Visual-QC workbench returned `401`, and both motherboard PM2 services remained online.
