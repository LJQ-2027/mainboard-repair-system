...
- [ ] **Step 1: Write failing package/report correlation tests**
...
- [ ] **Step 3: Implement exact-byte report and artifact validation**
...
- [ ] **Step 1: Write failing dry-run, transfer, resume, and conflict tests**
...
- [ ] **Step 3: Implement the handoff runner and receipt projection**
...
- [ ] **Step 1: Write failing same-byte transport tests**
...
- [ ] **Step 2: Run the two transport tests and verify RED**

Expected: the changed file is currently read and sent without a transport integrity error.

- [ ] **Step 3: Verify and reuse one byte string in multipart construction**
...
- [ ] **Step 4: Run intake and handoff suites and verify GREEN**
...
- [ ] **Step 4: Document the canonical owner workflow**
...
- [ ] Run all relevant source-library, audit, acceptance, handoff, and intake tests.
- [ ] Run the complete Python suite and require zero failures.
- [ ] Run the complete Node suite and require zero failures.
- [ ] Run Python compilation, JSON/Schema parsing, UTF-8/U+FFFD checks, and `git diff --check`.
- [ ] Run a temporary two-side stage -> audit -> acceptance -> handoff dry-run -> fake resumable transfer, compare source/acceptance snapshots before and after, and inspect receipt identity/status.
- [ ] Request independent code review; resolve each blocking finding through a new failing test followed by the minimal fix and rerun relevant/full suites.
