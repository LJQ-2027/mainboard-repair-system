...
内测反馈只记录“目标位置是否找到”和“资料是否足够继续”，保存在当前浏览器并由维修员手动导出 JSON。它与维修会话相互独立，不接收照片，不写入知识库，也不会自动成为维修事实。入口实现和验收记录见 `docs/technician-pilot-entry-2026-08-03.md`；维修会话实现记录见 `docs/technician-repair-session-2026-08-09.md`；首轮现场操作与回传规则见 `docs/technician-pilot-runbook-2026-08-09.md`。
...
生产已于 2026-08-13 升级到 `84b391713bc1ce34e8a75482435d6b2ec9d27430`，地址是 `https://cccsat.top/mb-repair-beta/`。维修员统一入口和来源受控维修工作台无需账号密码；内部视觉 QC API 与数据工作台继续受 Basic Auth 保护。该版本保留八板/14 机型入口及可恢复、可结束、可导出的维修员会话，并把维修流程重构为“现在做什么 -> 在主板上定位 -> 记录结果 -> 下一步”的任务优先界面。部署使用 71,965,886 字节不可变归档，SHA-256 为 `9ae5930b3478fb579f2cf5872b9dbebdf2be1e525bc9a33d53cb53963efe4a82`，并通过一致 SQLite 备份、完整迁移/回滚演练、公开入口和受保护数据面验证。操作与数据边界见 `docs/technician-pilot-runbook-2026-08-09.md`、`docs/visual-qc-capture-intake-spec-2026-07-20.md`、`docs/visual-qc-workbench-2026-07-17.md` 和 `docs/visual-qc-server-api-2026-07-20.md`。

生产部署使用 `VISUAL-QC-UPGRADE-PREFLIGHT-V1` 升级演练门禁和 `VISUAL-QC-DEPLOYMENT-MANIFEST-V1` 部署身份清单。`scripts/audit_visual_qc_upgrade.py` 只接受一致 SQLite 备份，在临时副本上验证受控对象、加法式迁移、Health/List/Detail/Dataset 契约、legacy 训练排除和旧版回读；源库与对象保持只读。真实部署会先将完整 40 位 commit、实际归档 SHA-256/字节数和归档内部运行时清单的 SHA-256 绑定为一个确定性清单，并上传到本次部署独有的只读暂存目录；本地清单只用于核对路径集合，Windows CRLF 不会替代 Git 归档中的真实字节身份。服务器在解包前拒绝重复 JSON 字段、身份不一致和危险 tar 成员，解包后拒绝符号链接、硬链接、特殊文件和运行时边界漂移，最后才停服备份和执行迁移演练。app/venv 只有在完整升级报告通过 JSON Schema、数据库快照和全部候选身份复核后才会切换。使用和证据边界见 `docs/visual-qc-upgrade-preflight-2026-07-23.md`。
