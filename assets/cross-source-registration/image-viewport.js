...
export function canStartImagePan({ interactive = false, button = 0 } = {}) {
...
export class ImageViewport {
  ...
  bind() {
    this.view.addEventListener('wheel', (event) => {
      ...
      const localState = {
        ...this.state,
        x: layerOrigin.x + this.state.x,
        y: layerOrigin.y + this.state.y,
      };
  ...
  zoomBy(factor) {
    ...
    const origin = this.layerOrigin();
    const zoomed = zoomAt(
      { ...this.state, x: origin.x + this.state.x, y: origin.y + this.state.y },
